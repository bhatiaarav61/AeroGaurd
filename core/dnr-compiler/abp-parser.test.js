...
function runTests() {
  ...
  test('parses domain list with negation', () => {
    // ABP spec uses | to separate domains in domain option
  ...
  test('recovers from malformed line', () => {
    ...
    assertEqual(result.rules.length, 3); // comment + 2 rules (malformed becomes comment or skipped)
  ...
  test('tracks stats correctly', () => {
    ...
    assertEqual(result.stats.skipped, 2); // 2 empty lines (comments are extracted but also counted as skipped in code)
