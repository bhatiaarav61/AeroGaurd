...
describe('ScriptletVerifier', () => {
  ...
  describe('Content Verification', () => {
    test('should allow safe scriptlet content without DOM access (SRI not enforced)', async () => {
      const safeContent = `
        (function() {
          const value = 1 + 2;
      ...
      `;
      // Create verifier with SRI not enforced for this test
      const verifierNoSRI = new ScriptletVerifier(mockWindow, {
        enforceSRI: false,
        enforceCSP: true,
        trustedTypesPolicy: 'test-policy',
        allowedHashes: new Set(),
      ...
      const result = await verifierNoSRI.verifyScriptlet('test-safe', safeContent);
      expect(result.verified).toBe(true);
    });

    test('should reject document access in strict mode', async () => {
      const domContent = `
        (function() {
      ...
      `;
      const result = await verifier.verifyScriptlet('test-dom', domContent);
  ...
  describe('CSP Compliance', () => {
    test('should verify CSP compliance for safe content without DOM access', async () => {
      const safeContent = `
        (function() {
          const value = 1 + 2;
          const result = value * 3;
...
describe('CSPUtils', () => {
  ...
  test('should check script allowance', () => {
    ...
    try {
      const csp = CSPUtils.parseCSP("script-src 'self' https://trusted.com https://example.com");
